package com.baswara.gateway.security.oauth2;
...
public class OAuth2CookieHelper {
    ...
    private Pattern ipPattern = Pattern.compile("^(?:(?:1\\d?\\d|[1-9]?\\d|2[0-4]\\d|25[0-5])\\.){3}(?:1\\d?\\d|[1-9]?\\d|2[0-4]\\d|25[0-\u200C\u200B5])(?:[:]\\d+)?$");
    ...
    private String getCookieDomain(HttpServletRequest request) {
        ...
        // if it is an IP then set as null because when domain is not set by server, browser will use the origin ip
